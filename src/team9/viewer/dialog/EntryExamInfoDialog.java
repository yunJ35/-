...
public class EntryExamInfoDialog extends JDialog implements SwingConstants {
	...
	public EntryExamInfoDialog(JFrame f) {
		super(f, " 添加考试信息", true);
		...
		btnOK.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				classCourse.setCouId(cbCourseId.getSelectedItem().toString());
				classCourse.setTeaId(cbTeacherId.getSelectedItem().toString());
				classCourse.setExamType(txtExamType.getText());
				classCourse.setExamDate(txtExamDate.getText());
				classCourse.setExamTime(txtExamTime.getText());
				classCourse.setExamAddr(txtExamAddr.getText());

				DButils.insertClassCourse(classCourse);
				lblMsg.setText("考试信息添加成功");
			}
		});
		...
		lblClassId.setBounds(panel.getSize().width / 4, 2 * panel.getSize().height / 12,
				2 * panel.getSize().width / 10, 35);
		cbClassId.setBounds(9 * panel.getSize().width / 20, 2 * panel.getSize().height / 12,
				3 * panel.getSize().width / 10, 35);
		lblCouId.setBounds(panel.getSize().width / 4, 3 * panel.getSize().height / 12,
				2 * panel.getSize().width / 10, 35);
		cbCourseId.setBounds(9 * panel.getSize().width / 20, 3 * panel.getSize().height / 12,
				3 * panel.getSize().width / 10, 35);
		lblTeaId.setBounds(panel.getSize().width / 4, 4 * panel.getSize().height / 12,
				2 * panel.getSize().width / 10, 35);
